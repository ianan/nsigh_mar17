pro make_nsgen_map,dir=dir,obsid=obsid,tbin=tbin,fextra=fextra,eps=eps,$
  ...
  if (n_elements(fextra) ne 1) then fextra='_cl_sunpos'
  if (n_elements(eps) ne 1) then eps=0
  if (n_elements(smooth) ne 1) then smooth=0
  if (n_elements(rebin) ne 1) then rebin=0
  ...
  for tt=0,num_ntims-1 do begin
    ...
    if keyword_set(rebin) then mapa=rebin_map(mapa,128,128)
    ...
    if keyword_set(rebin) then mapb=rebin_map(mapb,128,128)
    ...
    if keyword_set(eps) then begin
      ...
      device, /encapsulated, /color, / isolatin1,/inches,bits=8, xsize=9, ysize=4.5,$
        file='figs/'+fout+obsid+fextra+'_'+strmid(string(tbin+10000,format='(i5)'),1,4)+'_'+string(counter,format='(i5)')+'.eps'

      plot_map,mapa,/log,dmin=dmn,dmax=dmx,$
        ;        position=[0.17,0.13,0.87,0.83],$
        title=mapa.id,grid_sp=30,xrange=xr,yrange=yr,$
        gcolor=0,bot=1,chars=0.8,xtitle='x [arcsec]',ytitle='y [arcsec]'

      plot_map,mapb,/log,dmin=dmn,dmax=dmx,$
        ;        position=[0.17,0.13,0.87,0.83],$
        title=mapb.id,grid_sp=30,xrange=xr,yrange=yr,$
        gcolor=0,bot=1,chars=0.8,xtitle='x [arcsec]',ytitle='y [arcsec]'
